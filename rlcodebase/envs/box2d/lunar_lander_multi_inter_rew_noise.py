...
class LunarLander(gym.Env):
    ...
    def _step(self, action):
        ...
        if self.prev_shaping is not None:
            rewards[0] = shaping - self.prev_shaping + np.random.randn(1)*self.noise_level #Adding white noise
        self.prev_shaping = shaping

        # Rewards 1 and 2
        rewards[1] = - (m_power * 0.30) + np.random.randn(1)*self.noise_level  # less fuel spent is better, about -30 for heurisic landing
        rewards[2] = - (s_power * 0.03) + np.random.randn(1)*self.noise_level

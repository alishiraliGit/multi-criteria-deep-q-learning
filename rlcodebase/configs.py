...
def get_env_kwargs(env_name):
    if env_name.startswith('LunarLander'):
        ...
        kwargs = {
            ...
            'grad_norm_clipping': 10,
            'lander': False,
            'num_timesteps': 100000, #80001,  # default: 80001
            'env_wrappers': empty_wrapper,
            'exploration_schedule': None,
        }

    elif env_name.startswith('SepsisSim'):
        kwargs = {
            'optimizer_spec': dqn_utils.sepsissim_optimizer(),
            'q_func': dqn_utils.create_lander_q_network,
            'replay_buffer_size': 50000,
            'batch_size': 32,
            'gamma': 1.00,
            ...
            'grad_norm_clipping': 10,
            'lander': False,
            'num_timesteps': 100000,  # changed from 300k
            'env_wrappers': empty_wrapper,
            'ep_len': 200,
        }
        kwargs['exploration_schedule'] = dqn_utils.lander_exploration_schedule(kwargs['num_timesteps'])

    ...
    for consistency_alpha in consistency_alphas:
        for seed in seeds:
            command = \
                ...
                '--exp_name v4_var1c_%d_offline_pruned_cmdqn_lr[1]%.0e_tuf[1]%g_r[1]%g_lr[2]%.0e_tuf[2]%g_cql%g_alpha%g_sparse_eval' \
                ...
                '--arch_dim 32' + '\n' + \
                '--phase_2_critic_file_prefix v4_var1c_%d_offline_pruned_cmdqn_lr[1]%.0e_tuf[1]%g_r[1]%g_lr[2]%.0e_tuf[2]%g_cql%g_alpha%g_sparse_MIMIC' \
                % (seed, lr1, tuf1, r1, lr2, tuf2, cql_alpha, consistency_alpha) + '\n' + \
                '--pruning_file_prefix v3_var1c_%d_offline_cmdqn_lr%.0e_tuf%g_cql%g_alpha%g_r%g_MIMIC' \

...
def get_action_set_data(file, tag, printing=False, convert_to_list = True):
    
    # load pickle file
    with open(file, 'rb') as f:
        actions_dict_ = pickle.load(f)
    
    print(type(actions_dict_[tag]))
    print(len(actions_dict_[tag]))
    print(type(actions_dict_[tag][0]))
...
def plot_action_dist(phys_actions, pareto_actions, non_pareto_actions, eps='None',folderpath = "No path", figurepath=os.getcwd()):
    ...
    f, (ax1, ax3) = plt.subplots(1, 2, figsize=(8,4))
...
def plot_action_dist_batch(phys_actions, pareto_actions, non_pareto_actions, eps='None',folderpath = "No path", figurepath=os.getcwd()):
...
def preprocess_bins(traj_info,q_label='mean_q', baseline = False, bins=25):
    ...
    #IF WE WANT BINS BY absolute Q-value
    #bins = np.linspace(np.min(traj_info[q_label])*1.1, np.max(traj_info[q_label])*0.9, bins)
    #group = traj_info.groupby(pd.cut(traj_info.mean_q, q=bins))

    #IF WE WANT BINS BY Q-value percentile
    traj_info['perctiles'] = pd.qcut(traj_info.mean_q, q=bins,labels=False)
    group = traj_info.groupby('perctiles')
    print(group.mean().head(5))
    print(group.mean().shape)

    #plot_centers = (bins[:-1] + bins[1:]) / 2
    plot_centers = group.perctiles.mean()
    ...
    indices = group.survive.count() > 25
...
def plot_binned_mortality(file, eps, params, folder_path="test"):
    ...
    print(actions_dict_['q_vals'][0].shape)

    all_q_values = np.concatenate(actions_dict_['q_vals'], axis=0)
    all_rtgs = np.concatenate(actions_dict_['mortality_rtg'], axis=0)

    mean_q_per_traj = [np.mean(q_values_n) for q_values_n in actions_dict_['q_vals']]
    print(actions_dict_['q_vals'][0].shape)
    print(all_q_values.shape)
    print(mean_q_per_traj)

    print(actions_dict_['mortality_rtg'][0].shape)
    max_rtg_per_traj = [np.max(rtgs) for rtgs in actions_dict_['mortality_rtg'][0].tolist()]
    #print(max_rtg_per_traj)
    max_rtg_per_traj = [1 if np.max(rtgs) >0 else 0 for rtgs in actions_dict_['mortality_rtg'][0].tolist()]
    #print(max_rtg_per_traj)
    ...
    print(traj_info['survive'])

    rho = np.corrcoef(all_rtgs, all_q_values)[0, 1]
    print(rho)

    #print(actions_dict_.describe())
    ...
    plt.plot(plot_centers, plot_values, color="g", label=f'MCQL alpha={eps}') #f'{method} eps={eps}'
...
def plot_binned_mortality_batch(file_paths_, eps, params, folder_path="test"):

    fig_path_ = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', '..', 'figs')

    q_val_sets = [get_action_set_data(file, 'q_vals', convert_to_list=True) for file in file_paths_]
    mortality_sets = [get_action_set_data(file, 'mortality_rtg',convert_to_list=True) for file in file_paths_]

    print(len(q_val_sets))
    print(len(q_val_sets[0]))
    print(len(mortality_sets))
    print(len(mortality_sets[0]))
    all_q_values = np.concatenate(q_val_sets, axis=0)
    all_rtgs = np.concatenate(mortality_sets, axis=0)
    ...
    #max_rtg_per_traj = [np.max(rtgs) for rtgs in all_rtgs]
    #print(max_rtg_per_traj)

    survival = [1 if rtg >0 else 0 for rtg in all_rtgs.tolist()]
    ...
    #traj_info['mean_q'] = mean_q_per_traj
    traj_info['survive'] = survival
    print(traj_info['survive'][:20])
    ...
    #bins = np.linspace(np.min(traj_info['mean_q']), np.max(traj_info['mean_q']), 100)
    #bins = np.linspace(np.min(traj_info['mean_q'])*1.1, np.max(traj_info['mean_q'])*0.9, 50)
    plot_centers, plot_values, plot_ci, plot_range = preprocess_bins(traj_info, 'mean_q', False, bins=25)
    ...
    if params['save']:
        plt.savefig(os.path.join(fig_path_, folder_path + 'ALL_SEEDS_survival_by_Q.pdf'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prefix', type=str, default="v6_var1c_9_offline_pruned_cmdqn_alpha*_sparse_eval")
    ...
    #Offline learning
    # parser.add_argument('--buffer_path', type=str, default='./Replay_buffer_extraction/Encoded_paths13_all_rewards_var1.pkl')
    ...
    # TODO
    #eps_list = [int(f.split('_')[7][5:]) for f in folder_paths_short]
    eps_list_sorted = [int(f.split('_')[6][5:]) for f in folder_paths_short]
    ...
    #Batch version for algorithm
    print('Now batch version of action distribution, remember that the parsed models for this shall all be the same with different seeds.')
    phys_actions, policy_actions, non_pareto_actions = [], [], []
    eps = eps_list_sorted[i]
    for i in range(len(opt_action_sets)):
        phys_actions.extend(opt_action_sets[i])
        #phys_actions = unpack_actions(outputs_list[i]['opt_actions'])
        pareto_actions.extend(unpack_actions(pareto_action_sets[i]))
        #policy_actions = unpack_actions(outputs_list[i]['policy_actions'])
        policy_actions.extend(policy_action_sets[i])
        non_pareto_actions.extend(unpack_actions(pruned_action_sets[i])) #unpack_actions(pruned_action_sets[i])
        path_for_plot = folder_paths_short[i]
    
    path_for_plot = path_for_plot + '_ALL_SEEDS'
    print(path_for_plot)
    plot_action_dist(phys_actions, policy_actions, non_pareto_actions, eps, folderpath=path_for_plot, figurepath = fig_path_)
    ...
    ########################################################################
    #################### Survival by mean Q per trajectory  ################
    ########################################################################

    print('Survival by mean Q per trajectory')

    for i in range(len(opt_action_sets)):
        file = file_paths_[i]
        eps = eps_list_sorted[i]
        folder_path = folder_paths_short[i]
        plot_binned_mortality(file, eps, params, folder_path=folder_path)
    
    print('NOW ACROSS ALL SEEDS')
    plot_binned_mortality_batch(file_paths_, eps, params, folder_path=folder_path)
    ...
    for i in range(len(opt_action_sets)):
        ...
        phys_actions = opt_action_sets[i]
        eps = eps_list_sorted[i]
        #eps = 'TBD' #TODO make this track the alpha
    ...
    for i in range(len(sizes)):
        ...
        #plt.title(f'Pareto-set size distribution eps=TBD') #TODO make this track the alpha
    ...
    # TODO reactivate this when we have the eps list
    ...
    # TODO Consider reactivatin this 
    ...
    for results_df_grouped_mean, results_df_grouped_std in zip(results_df_grouped_means, results_df_grouped_stds):
        ...
        plt.xlabel('Pareto set size (# of actions)')
        #plt.title(f'Pareto-set accuracy by size eps=TBD') #TODO make this in future show the alpha
    ...
    for results_df_grouped_mean, results_df_grouped_std in zip(results_df_grouped_means, results_df_grouped_stds):
        ...
        #plt.title(f'Survival rate by number of pareto-actions in traj eps=TBD') #make this in future show the alpha
    ...
    for flagged, non_flagged in zip(flagged_dfs, non_flagged_dfs):
        ...
        #plt.title(f'Q-value historgram eps TBD') #make this in future show the alpha
    ...
    for flagged, non_flagged in zip(flagged_dfs, non_flagged_dfs):
        ...
        plt.ylabel('Probability')
        plt.xlabel('Mean Q-value per trajectory')
        #plt.title(f'Mean Q-value per traj historgram eps=TBD') #TODO make this in future show the alpha
